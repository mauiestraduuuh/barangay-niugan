...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (message) {
      const t = setTimeout(() => { setMessage(""); setMessageType(""); }, 5000);
      return () => clearTimeout(t);
    }
  }, [message]);

  const fetchRequests = async () => {
    if (!token) { setMessage("Unauthorized: No token found"); setInitialLoading(false); return; }
    setLoading(true);
    try {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      await axios.post(
        ...
        { certificate_type: certificateType.trim(), purpose: purpose.trim() || undefined },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (err.response?.status === 401) {
        ...
        setTimeout(() => { localStorage.removeItem("token"); router.push("/auth-front/login"); }, 2000);
      } else {
  ...
  if (initialLoading) {
    return (
      <div className="relative min-h-screen flex">
        <div className="w-20 bg-gray-100 animate-pulse flex flex-col p-4 gap-4">
        ...
        <div className="absolute inset-0 flex items-center justify-center z-20 backdrop-blur-sm bg-white/30">
          <div className="flex flex-col items-center gap-4">
            <div className="w-16 h-16 border-4 border-red-700 border-t-transparent rounded-full animate-spin" />
            <p className="text-gray-600 text-lg mt-3">Loading Certificate Requests...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-red-800 to-black p-4 flex gap-4">
      ...
      {/* Toast */}
      {message && (
        <div className={`fixed top-4 right-4 z-[200] max-w-md p-4 rounded-lg shadow-lg flex items-start gap-3 ${
          messageType === "success" ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"
        }`}>
          {messageType === "error" && <ExclamationTriangleIcon className="w-6 h-6 text-red-600 flex-shrink-0" />}
          <p className={`flex-1 ${messageType === "success" ? "text-green-800" : "text-red-800"}`}>{message}</p>
          <button onClick={() => { setMessage(""); setMessageType(""); }} className="text-gray-500 hover:text-gray-700">
      ...
      <div className={`${sidebarOpen ? "w-64" : "w-16"} bg-gray-50 shadow-lg rounded-xl transition-all duration-300 ease-in-out flex flex-col ${sidebarOpen ? "fixed inset-y-0 left-0 z-50 md:static" : "hidden md:flex"}`}>
        <div className="p-4 flex items-center justify-center">
          <img src="/niugan-logo.png" alt="Logo" className={`rounded-full object-cover transition-all duration-300 ${sidebarOpen ? "w-30 h-30" : "w-8.5 h-8.5"}`} />
          ...
          <ul>
            {features.map(({ name, label, icon: Icon }) => (
              <li key={name} className="mb-2">
                <Link href={`/dash-front/${name}`} onClick={() => setActiveItem(name)}
      ...
      {/* Main */}
      <div className="flex-1 flex flex-col gap-4 overflow-x-auto text-xs sm:text-sm md:text-base">
        ...
        <div className="bg-white p-5 rounded-xl shadow-lg flex flex-col gap-4">
          {/* Filter */}
          <div className="flex items-center justify-between flex-wrap gap-2">
            <h3 className="text-base font-semibold text-gray-800">Certificate History</h3>
            <div className="flex items-center gap-3">
              <label className="text-sm font-semibold text-gray-700">Filter Status:</label>
              <div className="relative">
                <select
                  className="appearance-none bg-white border border-gray-300 rounded-lg px-4 py-2 pr-8 text-sm font-medium text-gray-700 shadow-sm hover:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all"
                  value={statusFilter}
          ...
          {loading && !initialLoading ? (
            <div className="overflow-x-auto rounded-xl">
              <table className="w-full text-left border-collapse text-black">
                <thead className="bg-gradient-to-br from-black via-red-800 to-black text-white uppercase">
                  <tr>
                    {["ID", "Type", "Purpose", "Requested At", "Status", "Pickup Schedule", "Certificate"].map((h) => (
            ...
            <div className="overflow-x-auto rounded-xl">
              <table className="w-full text-left border-collapse text-black text-[0.65rem] sm:text-xs md:text-sm">
                <thead className="bg-gradient-to-br from-black via-red-800 to-black text-white uppercase">
                  <tr>
                    ...
                    <th className="p-4 text-center">Certificate</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedRequests.map((req) => (
                    <tr key={req.request_id} className="hover:bg-gray-50 transition duration-200 border-b">
                      ...
                      <td className="p-4 font-semibold">
                        {req.status === "REJECTED" ? (
                          <button
                            onClick={() => { setCurrentRejectionReason(req.rejection_reason || "No reason provided"); setShowRejectionModal(true); }}
                          ...
                          <span className={
                            ...
                            req.status === "CLAIMED" ? "text-purple-600" : "text-gray-600"
                          }>
                            {req.status}
                          </span>
                        )}
                      ...
                      <td className="p-4 text-center">
                        {req.file_path && (req.status === "APPROVED" || req.status === "CLAIMED") ? (
                          <a
                            href={req.file_path}
                            target="_blank"
            ...
            <div className="w-full mt-5 flex justify-center">
              <div className="flex items-center gap-2 px-3 py-1.5">
                <button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1}
                  className="px-2 py-1 text-3xl text-gray-500 hover:text-gray-700 disabled:opacity-30">‹</button>
                {Array.from({ length: totalPages }).map((_, i) => {
                  const page = i + 1;
                  if (page === 1 || page === totalPages || (page >= currentPage - 1 && page <= currentPage + 1)) {
                    return (
                      <button key={i} onClick={() => setCurrentPage(page)}
                        className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition-all ${currentPage === page ? "bg-red-700 text-white" : "text-gray-700 hover:bg-gray-100"}`}>
                        {page}
                      </button>
                    );
                  }
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl w-full max-w-md p-6 relative text-black">
            <button
              onClick={() => { if (!actionLoading) { setShowModal(false); setCertificateType(""); setPurpose(""); } }}
            ...
            <form onSubmit={handleRequestSubmit} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-red-700 focus:outline-none"
                  placeholder="e.g. Scholarship Application, Employment..."
                  disabled={actionLoading}
                />
              </div>
              <button
      ...
      {showRejectionModal && (
        <div className="fixed inset-0 z-[1000] bg-black bg-opacity-40 flex items-center justify-center">
          <div className="bg-white rounded-xl w-full max-w-sm p-6 relative">
            <button onClick={() => setShowRejectionModal(false)} className="absolute top-4 right-4 text-black hover:text-red-700">
              <XMarkIcon className="w-6 h-6" />
            </button>
            <h2 className="text-xl font-semibold mb-4 text-red-600">Request Denied</h2>
            <p className="text-gray-700">{currentRejectionReason}</p>
            <button onClick={() => setShowRejectionModal(false)} className="mt-4 bg-red-700 text-white px-4 py-2 rounded-lg hover:bg-red-800">
              Close
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[1000] bg-black bg-opacity-40 flex items-center justify-center">
          <div className="bg-white rounded-xl w-full max-w-md p-6 relative">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowConfirmModal(false)} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={handleConfirmSubmit} className="px-4 py-2 bg-red-700 text-white rounded-lg hover:bg-red-800">
                Confirm
              </button>
            </div>
          </div>
