 ...
 const handleLogout = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-red-800 to-black p-4 flex gap-4">
      {actionLoading && <LoadingOverlay message={activeSection === "edit" ? "Updating profile..." : "Changing password..."} />}
      {showConfirmModal && (
        <ConfirmationModal
      ...
      {/* Sidebar */}
      <div
        className={`${sidebarOpen ? "w-64" : "w-16"} bg-gray-50 shadow-lg rounded-xl transition-all duration-300 flex flex-col ${
            ...
            {features.map(({ name, label, icon: Icon }) => {
              ...
              return (
                <li key={name} className="mb-2">
                  <Link href={`/staff-front/${name}`} aria-current={isActive ? "page" : undefined}>
                    <span
