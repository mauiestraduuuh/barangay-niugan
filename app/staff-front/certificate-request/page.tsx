...
const LoadingSpinner = ({ size = "md" }: { size?: "sm" | "md" | "lg" }) => {
  const cls = { sm: "w-4 h-4 border-2", md: "w-8 h-8 border-3", lg: "w-12 h-12 border-4" };
  return <div className={`${cls[size]} border-red-700 border-t-transparent rounded-full animate-spin`} />;
};
...
export default function AdminCertificateRequestsPage() {
  ...
  const [activeItem] = useState("certificate-request");
  ...
  // Newly generated certificate URL shown in a success banner
...
const generateCertificate = async (requestId: string): Promise<string | null> => {
  ...
  try {
    const res = await axios.post(
      "/api/staff/generate-certificate",
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      ...
      await fetchRequests();
      setMessage("Request approved successfully! Certificate generated.");
      if (certUrl) setGeneratedCertUrl(certUrl);
    } catch (err) {
      console.error(err);
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, string> = {
      ...
      CLAIMED: "bg-purple-200 text-purple-800",
    };
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${map[status] || "bg-gray-200 text-gray-800"}`}>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-red-800 to-black p-4 flex gap-4">
            ...
            <>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {req.file_path && (
                        <a href={req.file_path} target="_blank" rel="noopener noreferrer"
                          className="bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1.5 rounded flex items-center gap-1 text-xs">
                          <DocumentArrowDownIcon className="w-4 h-4" /> Certificate
                        </a>
                      )}
                      ...
                      <tr key={req.request_id} className={`border-b hover:bg-red-50 transition ${index % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                        ...
                        <td className="px-3 py-2">
                          <div className="flex flex-wrap gap-1">
                            ...
                            {/* Certificate download button — shown when file_path exists */}
                            {req.file_path && (
                              <a
                                href={req.file_path}
                                target="_blank"
              ...
              <div className="w-full mt-5 flex flex-col sm:flex-row justify-center items-center gap-3">
                <div className="flex items-center gap-1 sm:gap-2">
                  <button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} className="px-2 py-1 text-2xl sm:text-3xl text-gray-500 hover:text-gray-700 disabled:opacity-30">‹</button>
                  {Array.from({ length: totalPages }).map((_, i) => {
                    ...
                    if (page === currentPage - 2 || page === currentPage + 2) {
                      return <div key={i} className="px-1 text-gray-400 text-xs sm:text-base">...</div>;
                    }
                    return null;
                  })}
                  <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="px-2 py-1 text-2xl sm:text-3xl text-gray-500 hover:text-gray-700 disabled:opacity-30">›</button>
                </div>
                <select
                  value={ITEMS_PER_PAGE}
                  onChange={(e) => { setCurrentPage(1); setITEMS_PER_PAGE(Number(e.target.value)); }}
          ...
          <div className="bg-white p-4 sm:p-6 rounded-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            ...
            {confirmAction === "APPROVE" && (
              <p className="text-sm text-green-700 bg-green-50 border border-green-200 rounded p-2 mb-4">
                ℹ️ A certificate will be automatically generated and attached to this request upon approval.
              </p>
            )}
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setConfirmOpen(false)} className="px-4 py-2 rounded bg-gray-300 hover:bg-gray-400">No</button>
              <button
                onClick={() => {
                ...
                className={`px-4 py-2 rounded text-white ${
                  confirmAction === "REJECT" && !remarks.trim() ? "bg-red-300 cursor-not-allowed" : "bg-red-500 hover:bg-red-600"
                }`}
              >
                Yes
              </button>
      ...
      {/* ── Reject Confirm Modal (from modal flow) ── */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-4 sm:p-6 rounded-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 mt-4">
              <button onClick={() => setRejectConfirmOpen(false)} className="w-full sm:w-auto px-4 py-2 rounded bg-gray-300 hover:bg-gray-400">Cancel</button>
              <button
                onClick={async () => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 text-black">
          <div className="bg-white p-4 sm:p-6 rounded-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            ...
            {modalType === "CLAIMED" && (
              ...
              <p className="font-semibold">{selectedRequest.resident.first_name} {selectedRequest.resident.last_name}</p></>
            )}

            {modalType === "REJECT" && (
              <textarea className="w-full border p-2 rounded mb-2" placeholder="Rejection Reason" value={remarks} onChange={(e) => setRemarks(e.target.value)} />
            )}
            ...
            {modalType === "VIEW" && (
              <div className="space-y-2 text-sm">
                ...
                {selectedRequest.pickup_date && <p><strong>Pickup Date:</strong> {selectedRequest.pickup_date} {selectedRequest.pickup_time}</p>}
                ...
                {/* Certificate download link in view modal */}
                {selectedRequest.file_path && (
                  <div className="mt-3 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                    <p className="text-emerald-800 font-semibold mb-1 text-sm">📄 Certificate Available</p>
                    <a
                ...
                {/* Regenerate button for approved requests without a certificate */}
            ...
            {modalType === "ATTACH" && (
              <>
                <textarea className="w-full border p-2 rounded mb-2" placeholder="Remarks" value={remarks} onChange={(e) => setRemarks(e.target.value)} />
                <input type="file" onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="mb-4" />
              </>
            )}

            <div className="flex justify-end gap-2 mt-4">
              <button onClick={closeModal} className="px-4 py-2 rounded bg-gray-300 hover:bg-gray-400" disabled={actionLoading}>Close</button>
              {modalType !== "VIEW" && (
                <button
