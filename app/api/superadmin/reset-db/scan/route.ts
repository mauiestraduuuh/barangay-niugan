...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const activeResidentIds = [
      ...new Set([
        ...activeByCert.map((r) => r.resident_id).filter((id): id is number => id !== null),
        ...activeByFeedback.map((r) => r.resident_id).filter((id): id is number => id !== null),
        ...activeByClearance.map((r) => r.resident_id).filter((id): id is number => id !== null),
      ]),
    ];

    const inactiveResidents = await prisma.resident.findMany({
      where: activeResidentIds.length > 0 ? { resident_id: { notIn: activeResidentIds } } : {},
      select: {
        resident_id: true,
        first_name:  true,
        last_name:   true,
    ...
    const activeUserIds = [
      ...new Set([
        ...approvedCerts.map((r) => r.approved_by).filter((id): id is number => id !== null),
        ...claimedCerts.map((r) => r.claimed_by).filter((id): id is number => id !== null),
        ...respondedFeedbacks.map((r) => r.responded_by).filter((id): id is number => id !== null),
        ...issuedClearances.map((r) => r.issued_by).filter((id): id is number => id !== null),
        ...postedAnnouncements.map((r) => r.posted_by).filter((id): id is number => id !== null),
        ...createdEvents.map((r) => r.created_by).filter((id): id is number => id !== null),
      ]),
    ];
    ...
    const inactiveStaff = await prisma.staff.findMany({
      where: activeStaffIds.length > 0 ? { staff_id: { notIn: activeStaffIds } } : {},
      select: {
        staff_id:   true,
        first_name: true,
        last_name:  true,
